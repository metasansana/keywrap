...
var UnknownTypeError = require('./UnknownTypeError');
		...
		for (var key in fields)
			if (fields.hasOwnProperty(key)) {
				...
				if (_isString(current))
					converted[key] =
					(_checkType(types[current])) ?
					types[current] :
					_throw(new UnknownTypeError(current));

				if (_isString(current.type))
					converted[key] =
					(_checkType(types[current.type])) ?
					_swap(current, types[current.type]) :
					_throw(new UnknownTypeError(current.type));
